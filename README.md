...
2. その記事と、関連する記事(関連の強いものから抽選で40件)が3Dグラフになる
...
6. 中心(現在地)の球をクリックすると、関連記事が8件ずつ増える(1記事あたり40件まで)。
   右のサイドバーの「隣接記事」の横にある `+ MORE 8` ボタンでも同じ。
   増やした分は、別の記事へ進んで戻ってきても残る(URL には残らない)
7. 左下のパンくず、← ボタン、Backspace キーで来た道を戻れる
8. 右上の ↺ ボタンでグラフを空にして最初からやり直せる
...
- **名前(ラベル)** = 現在地より奥にある記事ほど薄くなり、ずっと奥では消える。
  ドラッグで回して手前に持ってくると読める(球と線は奥でも見えている)。
  `?preset=current` か `?labelDepthFade=0` で従来どおり全部同じ濃さになる

関連の強さは、次の3つを足したスコアで決めています。

- 本文の内容が近いか(Wikipedia の検索エンジンの `morelike:`)
- 相手の記事からもこの記事へリンクしているか(相互リンク)
- この記事の冒頭(リード文・インフォボックス)でリンクされているか

内容の近さだけだと、キャラクター記事に対する担当声優のように、書かれている言葉は違うが
